# Copyright 2020, General Electric Company. All rights reserved. See https://github.com/xcist/code/blob/master/LICENSE

import numpy as np
from catsim.C_DD3Proj_roi_notrans_mm import DD3Proj_roi_notrans_mm

# --------- Set parameters
# source coordinates
x0 = 0
y0 = 550
z0 = 0

# detector dimension and center coordinates
nrdetcols = 200
nrdetrows = 16
xds = np.linspace(-nrdetcols / 2 + 0.5, nrdetcols / 2 - 0.5, nrdetcols, dtype=np.single)
yds = -400 * np.ones(nrdetcols, dtype=np.single)
zds = np.linspace(-nrdetrows / 2 + 0.5, nrdetrows / 2 - 0.5, nrdetrows, dtype=np.single)

# original image and view settings
dx = 1
dz = 1
imgXoffset = 0
imgYoffset = 0
imgZoffset = 0
nrviews = 315
# view angle of each view
viewangles = np.linspace(0, np.pi * 2, nrviews, dtype=np.single)
# z-position of each view
zshifts = np.zeros(nrviews, dtype=np.single)

nrcols = 51
nrrows = 51
nrplanes = 30
pOrig = np.zeros((nrrows, nrcols, nrplanes), dtype=np.single)
pOrig[25, 25, :] = 1
pOrig[25, 0, :] = 1
pOrig[0, 0, :] = 1
mask = np.ones((nrcols, nrrows * 2), dtype=np.uint8)

# --------- Run DD3Proj
sinogram = DD3Proj_roi_notrans_mm(
    x0,
    y0,
    z0,
    nrdetcols,
    nrdetrows,
    xds,
    yds,
    zds,
    imgXoffset,
    imgYoffset,
    imgZoffset,
    viewangles,
    zshifts,
    nrviews,
    nrcols,
    nrrows,
    nrplanes,
    pOrig,
    dx,
    dz,
    mask,
)

# --------- Show results
import matplotlib.pyplot as plt

plt.imshow(sinogram[:, :, 7])
plt.show()
